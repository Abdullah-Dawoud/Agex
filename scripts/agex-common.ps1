...
function Get-AgexRuntimeIdentity {
...
function Write-AgexRuntimeIdentityNotice {
...
function ConvertTo-AgexAsciiJson {
    # Escapes every non-ASCII character as \uXXXX. JSON parsers restore the
    # exact text, and no console code page on the way can corrupt it.
    param([Parameter(Mandatory)][AllowEmptyString()][string]$Json)
    [regex]::Replace($Json, '[^\x00-\x7F]', { param($m) '\u{0:x4}' -f [int][char]$m.Value })
}

function Get-AgexVersion {
    $file = Join-Path (Split-Path -Parent $PSScriptRoot) "VERSION"
    if (Test-Path -LiteralPath $file -PathType Leaf) { return ([IO.File]::ReadAllText($file)).Trim() }
    "0.0.0-dev"
}

function Get-AgexDataRoot {
    # All AGEX-owned state lives here (settings, projects, sessions, logs,
    # telemetry, worker runs, scan cache). Nothing is written into the repo or
    # into the Codex home folder.
    $base = if ($env:AGEX_HOME) { $env:AGEX_HOME } else { Join-Path $env:LOCALAPPDATA "AGEX" }
    if (-not (Test-Path -LiteralPath $base -PathType Container)) { [void][IO.Directory]::CreateDirectory($base) }
    $base
}

function Get-AgexPath {
    param([Parameter(Mandatory)][ValidateSet("Settings", "Projects", "Sessions", "Logs", "Telemetry", "Workers", "Scan", "Cache", "Reports")][string]$Name)
    $root = Get-AgexDataRoot
    $path = switch ($Name) {
        "Settings" { Join-Path $root "settings.json" }
        "Projects" { Join-Path $root "projects.json" }
    ...
    if ($Name -notin @("Settings", "Projects", "Scan") -and -not (Test-Path -LiteralPath $path -PathType Container)) { [void][IO.Directory]::CreateDirectory($path) }
    $path
}

function Initialize-AgexStorage {
    # One-time migration from the pre-AGEX layout (settings and project list
    # under the Codex home folder). Old files are left in place and never
    # written again. See docs/INSTALL.md "Migration".
    $root = Get-AgexDataRoot
    $marker = Join-Path $root "migration.json"
    if (Test-Path -LiteralPath $marker -PathType Leaf) { return }
    # A custom AGEX_HOME is an isolated profile (tests, portable use): never import.
    if ($env:AGEX_HOME) { try { [IO.File]::WriteAllText($marker, '{"migrated":[],"reason":"custom AGEX_HOME"}') } catch { }; return }
    $codexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $env:USERPROFILE ".codex" }
    $migrated = @()
    $legacySettings = Join-Path $codexHome "dawoud-settings.json" # pre-AGEX name; migration only
    $settings = Get-AgexPath Settings
    if (-not (Test-Path -LiteralPath $settings) -and (Test-Path -LiteralPath $legacySettings -PathType Leaf)) { Copy-Item -LiteralPath $legacySettings -Destination $settings; $migrated += "settings" }
    $legacyProjects = Join-Path $codexHome "workbench-projects.json"
    $projects = Get-AgexPath Projects
    if (-not (Test-Path -LiteralPath $projects) -and (Test-Path -LiteralPath $legacyProjects -PathType Leaf)) { Copy-Item -LiteralPath $legacyProjects -Destination $projects; $migrated += "projects" }
    try { [IO.File]::WriteAllText($marker, (@{ migrated_at = (Get-Date).ToUniversalTime().ToString("o"); items = $migrated } | ConvertTo-Json -Compress), [Text.UTF8Encoding]::new($false)) } catch { }
}

function Get-AgexPreferences {
    param([string]$Path)
    if (-not $Path) { Initialize-AgexStorage; $Path = Get-AgexPath Settings }
    $defaults = [ordered]@{
        version = 3
        ...
        antigravity_model = ""
        antigravity_effort = ""
        codex_task_sandbox = "read-only"
        enabled_agents = @("codex", "antigravity")
        codex_timeout_seconds = 900
        antigravity_timeout_seconds = 840
        max_sessions = 200
        check_updates = $true
    ...
    if (Test-Path -LiteralPath $Path -PathType Leaf) {
        try {
            ...
            if ($null -eq $saved.PSObject.Properties["first_run_complete"] -and $saved.PSObject.Properties["last_project"] -and $saved.last_project) { $defaults.first_run_complete = $true }
        } catch { }
    }
    $defaults.last_modes = @($defaults.last_modes | ForEach-Object { [string]$_ })
    $defaults.recent_projects = @($defaults.recent_projects | Where-Object { $_ } | ForEach-Object { [string]$_ } | Select-Object -First 12)
    $defaults.enabled_agents = @($defaults.enabled_agents | Where-Object { $_ } | ForEach-Object { ([string]$_).ToLowerInvariant() } | Select-Object -Unique)
    $defaults.leader = switch ([string]$defaults.leader) { "Antigravity" { "Antigravity" } "Auto" { "Auto" } default { "Codex" } }
    if ([string]$defaults.strategy -notin @("Balanced", "Coding-heavy", "Research-heavy", "Custom")) { $defaults.strategy = "Custom" }
    if ([string]$defaults.codex_task_sandbox -notin @("read-only", "workspace-write")) { $defaults.codex_task_sandbox = "read-only" }
    if ([string]$defaults.theme -notin @("Light", "Dark")) { $defaults.theme = "Light" }
    foreach ($key in @("codex_timeout_seconds", "antigravity_timeout_seconds", "max_sessions")) { try { $defaults[$key] = [int]$defaults[$key] } catch { $defaults[$key] = 900 } }
    if ($defaults.codex_timeout_seconds -lt 60) { $defaults.codex_timeout_seconds = 900 }
    if ($defaults.antigravity_timeout_seconds -lt 60) { $defaults.antigravity_timeout_seconds = 840 }
    if ($defaults.max_sessions -lt 10) { $defaults.max_sessions = 200 }
    foreach ($key in @("first_run_complete", "check_updates", "start_with_windows", "launch_minimized")) { $defaults[$key] = [bool]$defaults[$key] }
...
function Get-AgexStrategyShares {
    # Strategy presets are preference weights for routing, not hard quotas.
    param([string]$Strategy, [int]$CodexShare = 20)
    switch ($Strategy) {
        "Balanced" { return @{ Codex = 50; Antigravity = 50 } }
        "Coding-heavy" { return @{ Codex = 20; Antigravity = 80 } }
        "Research-heavy" { return @{ Codex = 70; Antigravity = 30 } }
        default { return @{ Codex = $CodexShare; Antigravity = 100 - $CodexShare } }
    }
}

function Save-AgexPreferences {
    param([Parameter(Mandatory)]$Preferences, [string]$Path)
    if (-not $Path) { $Path = Get-AgexPath Settings }
    $parent = Split-Path -Parent $Path
    if ($parent -and -not (Test-Path -LiteralPath $parent)) { New-Item -ItemType Directory -Path $parent -Force | Out-Null }
    $safe = [ordered]@{}
    $template = Get-AgexPreferences -Path ([IO.Path]::Combine($env:TEMP, "agex-missing-" + [guid]::NewGuid().ToString("N") + ".json"))
    foreach ($key in $template.PSObject.Properties.Name) {
        $safe[$key] = if ($Preferences.PSObject.Properties[$key]) { $Preferences.$key } else { $template.$key }
    }
    $safe.version = 3
    $safe.recent_projects = @($safe.recent_projects | Where-Object { $_ } | Select-Object -Unique -First 12)
    $safe.enabled_agents = @($safe.enabled_agents)
...
function Add-AgexRecentProject {
    param([Parameter(Mandatory)]$Preferences, [Parameter(Mandatory)][string]$ProjectPath)
    $Preferences.last_project = $ProjectPath
    $Preferences.recent_projects = @(@($ProjectPath) + @($Preferences.recent_projects | Where-Object { $_ -and $_ -ne $ProjectPath }) | Select-Object -First 12)
}
...
function Receive-AgexWebSocketJson {
...
function Send-AgexWebSocketJson {
...
function Get-CodexModelCatalog {
    ...
    try {
        ...
        Send-AgexWebSocketJson -Socket $socket -CancellationToken $cts.Token -Payload @{ jsonrpc = "2.0"; id = 1; method = "initialize"; params = @{ clientInfo = @{ name = "agex"; version = "1" }; capabilities = @{} } }
        $request = $null
        for ($i = 0; $i -lt 12; $i++) {
            $message = Receive-AgexWebSocketJson -Socket $socket -CancellationToken $cts.Token
            if ($message.id -eq 1) {
                Send-AgexWebSocketJson -Socket $socket -CancellationToken $cts.Token -Payload @{ jsonrpc = "2.0"; id = 2; method = "model/list"; params = @{ includeHidden = $false; limit = 100 } }
            }
...
function Resolve-AgyExecutable {
    ...
    # Explicit override for non-standard installs and for isolated tests.
    if ($env:AGEX_AGY_PATH) { [void]$candidates.Add($env:AGEX_AGY_PATH) }
...
function Test-AgexAgyAuthFailure {
...
function Invoke-AgexAgyStream {
    ...
    try {
        ...
        # Write UTF-8 bytes directly. StandardInput.WriteLine uses the console
        # code page on Windows PowerShell 5.1 and corrupts non-ASCII prompts.
        ...
        while ($null -eq $result) {
            ...
            if ($firstStdout -eq "NONE") { $firstStdout = Protect-AgexTelemetryText -Text $line; & $milestone "FIRST_STDOUT_EVENT" }
            [void]$stdoutLines.Add((Protect-AgexAuthoritativeText -Text $line))
            if ($RawStdoutPath) { Add-Content -LiteralPath $RawStdoutPath -Value (Protect-AgexAuthoritativeText -Text $line) -Encoding utf8 }
            ...
            elseif ($event.event -eq "error" -and $event.error) { $failureReason = Protect-AgexTelemetryText -Text ([string]$event.error) }
        ...
        if ($stderr.Trim()) { [void]$stderrLines.Add((Protect-AgexTelemetryText -Text $stderr.Trim())) }
        ...
        $exceptionMessage = Protect-AgexTelemetryText -Text $_.Exception.Message
        ...
        try {
            ...
            $streamInfo = {
                ...
                } catch { $info.ProbeError = Protect-AgexTelemetryText -Text $_.Exception.Message }
                if ($reader -is [IO.StreamReader]) { try { $info.EndOfStream = $reader.EndOfStream } catch { $info.ProbeError = Protect-AgexTelemetryText -Text $_.Exception.Message } }
                [pscustomobject]$info
            }
            $stdout = $null; $stderr = $null; $stdin = $null; $hasExited = $null; $processProbeError = ""
            try { if ($proc.StartInfo.RedirectStandardOutput) { $stdout = & $streamInfo $proc.StandardOutput } } catch { $processProbeError = Protect-AgexTelemetryText -Text $_.Exception.Message }
            try { if ($proc.StartInfo.RedirectStandardError) { $stderr = & $streamInfo $proc.StandardError } } catch { $processProbeError = Protect-AgexTelemetryText -Text $_.Exception.Message }
            try { if ($proc.StartInfo.RedirectStandardInput) { $stdin = & $streamInfo $proc.StandardInput } } catch { $processProbeError = Protect-AgexTelemetryText -Text $_.Exception.Message }
            try { $hasExited = $proc.HasExited } catch { $processProbeError = Protect-AgexTelemetryText -Text $_.Exception.Message }
            $runspaceId = ""
            ...
            $failureDiagnostic = [ordered]@{
                ...
                inner_exception_message = if ($eventProcessingError) { Protect-AgexTelemetryText -Text $eventProcessingError.Exception.Message } else { "" }
    ...
    [pscustomobject]@{
        Contract = "AGEX_AGY_RESULT_V1"; Success = $success; ActualExe = $AgyPath; Arguments = @($args); ActualPid = if ($processStarted) { $proc.Id } else { 0 }; ProcessStarted = $processStarted
...
function Get-AgexAgyAvailability {
    ...
    try {
        ...
        $result.Version = Protect-AgexTelemetryText -Text $versionOutput
    ...
    try {
        $probeLog = Join-Path $env:TEMP ("agex-agy-probe-" + $PID + ".log")
        ...
        if ($result.ModelsExitCode -ne 0) {
            $result.Reason = "agy.exe models failed with exit code $($result.ModelsExitCode): $(Protect-AgexTelemetryText -Text $modelsOutput)"
            return [pscustomobject]$result
        }
...
function Get-AntigravityModelCatalog {
    ...
    if (-not $agy) { return @() }
    try {
        $probeLog = Join-Path $env:TEMP ("agex-agy-catalog-" + $PID + ".log")
...
function Get-AgexRouteDecision {
    param(
    ...
    $category = Get-AgexTaskCategory -Task $Task
    $reason = ""
    $agent = "Codex"
    ...
    elseif ($TelemetryRoot -and $SessionId -and $AntigravityShare -gt $CodexShare) {
        $budget = Get-AgexWorkloadBudget -TelemetryRoot $TelemetryRoot -SessionId $SessionId -CodexShare $CodexShare -AntigravityShare $AntigravityShare
...
function Get-AgexWorkloadBudget {
    param([Parameter(Mandatory)][string]$TelemetryRoot, [Parameter(Mandatory)][string]$SessionId, [int]$CodexShare, [int]$AntigravityShare)
    $records = @(Get-AgexTelemetryRecords -TelemetryRoot $TelemetryRoot -SessionId $SessionId | Where-Object { $_.RecordKind -eq "TASK" })
...
function Protect-AgexTelemetryText {
...
function Protect-AgexAuthoritativeText {
    param([AllowEmptyString()][string]$Text)
    if ($null -eq $Text) { return '' }
    $safe = $Text -replace '(?i)(api[_-]?key|token|password|secret|cookie|authorization)\s*[:=]\s*[^\s,;]+', '$1=<redacted>'
...
function Write-AgexTelemetryJson {
    param(
...
function New-AgexTelemetryRecord {
    param(
    ...
    $record = [ordered]@{
        TaskId = $id
        SessionId = $SessionId
        Task = Protect-AgexTelemetryText -Text $Task
        WorkId = $WorkId
...
function Complete-AgexTelemetryRecord {
    param(
        [Parameter(Mandatory)][string]$Path,
    ...
    $record | Add-Member -MemberType NoteProperty -Name Summary -Value (Protect-AgexTelemetryText -Text $Summary) -Force
    $json = $record | ConvertTo-Json -Depth 8
    if (-not (Write-AgexTelemetryJson -Path $Path -Json $json)) {
        throw "Telemetry file write failed after bounded retries: $Path"
    }
...
function Get-AgexTelemetryRecords {
...
function Get-AgexExecutionReport {
    ...
    $records = @(Get-AgexTelemetryRecords -TelemetryRoot $TelemetryRoot -SessionId $SessionId -WorkId $WorkId)
