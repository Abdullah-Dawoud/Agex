...
. "$PSScriptRoot/agex-ui.ps1"
. "$PSScriptRoot/agex-graph.ps1"
. "$PSScriptRoot/agex-acceptance-fixture.ps1"
function Assert-Graph($condition,$message) { if (-not $condition) { throw $message } }
$Project=$PSScriptRoot
$script:ui=New-AgexUiState -Project $Project -SessionId 'truth-test'
$workspacePrompt=New-AgexExecutorPrompt -RootGoal 'Create artifact' -Entry ([pscustomobject]@{Task='Write proof';AffectedFiles=@('proof.txt')}) -Workspace 'C:\fixture workspace' -Mail '[]' -Context '[]'
...
try {
    ...
    Assert-Graph (-not (Test-AgexAgyAuthFailure -CliLogPath $authLog)) 'Successful silent auth must supersede early auth warning'
    Add-Content -LiteralPath $authLog -Value 'authentication required'
    Assert-Graph (Test-AgexAgyAuthFailure -CliLogPath $authLog) 'Final auth failure must remain an executor blocker'
} finally { Remove-Item -LiteralPath $authLog -Force -ErrorAction SilentlyContinue }
$absent=Get-AgexTaskVerification ([pscustomobject]@{AffectedFiles=@('never-created-artifact.txt');Result='Created never-created-artifact.txt'})
Assert-Graph (-not $absent.Pass -and -not $absent.Evidence[0].exists) 'False file claim must fail verification'
$verifiedPath=Join-Path $PSScriptRoot '.agex-verified-fixture.txt'
try {
    [IO.File]::WriteAllText($verifiedPath,'actual output')
    $present=Get-AgexTaskVerification ([pscustomobject]@{AffectedFiles=@('.agex-verified-fixture.txt');Result='Created .agex-verified-fixture.txt'})
...
Assert-Graph (Test-AgexPathConflict @{AffectedFiles=@('src')} @{AffectedFiles=@('src/a.ps1')}) 'Directory overlap missed'
Assert-Graph (Test-AgexPathConflict @{AffectedFiles=@()} @{AffectedFiles=@('a.ps1')}) 'Unknown ownership must serialize'
Assert-Graph (-not (Test-AgexPathConflict @{AffectedFiles=@('a.ps1')} @{AffectedFiles=@('b.ps1')})) 'Independent files conflict'
$rejected=$false
try { ConvertFrom-AgexPlan '{"goal_status":"COMPLETE","tasks":[]}' } catch { $rejected=$true }
...
$plan=ConvertFrom-AgexPlan (@{goal_status='CONTINUE';tasks=$many} | ConvertTo-Json -Depth 5)
Assert-Graph (@($plan.tasks).Count -eq $many.Count) 'Task truncation'
$auditState=New-AgexUiState -Project $Project -SessionId 'plan-audit-test'
...
$duplicate=ConvertFrom-AgexPlan -Text $duplicatePlan -AuditState $auditState
...
$missing=ConvertFrom-AgexPlan -Text '{"goal_status":"CONTINUE","tasks":[{"title":"No id","objective":"Read","executor":"Codex"}]}'
Assert-Graph ($missing.tasks[0].id -eq 'task-0001') 'Missing leader id must receive canonical id'
$temporary=ConvertFrom-AgexPlan -Text '{"goal_status":"CONTINUE","tasks":[{"id":"inspect","title":"Inspect","objective":"Read","executor":"Codex"},{"id":"verify","title":"Verify","objective":"Check","executor":"Codex","dependencies":["Inspect"]}]}'
Assert-Graph ($temporary.tasks[1].dependencies -eq 'task-0001') 'Temporary dependency label must map to canonical id'
$unknownRejected=$false;try{ConvertFrom-AgexPlan -Text '{"goal_status":"CONTINUE","tasks":[{"title":"A","objective":"Read","executor":"Codex","dependencies":["missing"]}]}'|Out-Null}catch{$unknownRejected=$_.Exception.Message -match "Unknown dependency 'missing'"}
Assert-Graph $unknownRejected 'Unknown dependency must report exact target'
$selfRejected=$false;try{ConvertFrom-AgexPlan -Text '{"goal_status":"CONTINUE","tasks":[{"id":"self","title":"Self","objective":"Read","executor":"Codex","dependencies":["self"]}]}'|Out-Null}catch{$selfRejected=$_.Exception.Message -match 'Self dependency on task-0001'}
Assert-Graph $selfRejected 'Self dependency must report canonical task'
$cycleRejected=$false;try{ConvertFrom-AgexPlan -Text '{"goal_status":"CONTINUE","tasks":[{"id":"a","title":"A","objective":"Read A","executor":"Codex","dependencies":["b"]},{"id":"b","title":"B","objective":"Read B","executor":"Codex","dependencies":["a"]}]}'|Out-Null}catch{$cycleRejected=$_.Exception.Message -match 'Dependency cycle: task-0001 -> task-0002 -> task-0001'}
...
$followup=ConvertFrom-AgexPlan -Existing @($existingRepair) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair","title":"Repair","objective":"Fix","executor":"Codex","repair_for":["task-0001"]}]}'
...
$initialPlan=ConvertFrom-AgexPlan -Text '{"goal_status":"CONTINUE","tasks":[{"id":"implement-utility-and-readme","title":"Implement utility and README","objective":"Create files","executor":"Antigravity"}]}'
...
$repairByOriginal=ConvertFrom-AgexPlan -Existing @($existingTask) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair-original","title":"Repair original","objective":"Fix files","executor":"Antigravity","repair_for":["implement-utility-and-readme"]}]}'
Assert-Graph ($repairByOriginal.tasks[0].repair_for -eq 'task-0001') 'Original model repair id must resolve to canonical task'
$repairByCanonical=ConvertFrom-AgexPlan -Existing @($existingTask) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair-canonical","title":"Repair canonical","objective":"Fix files","executor":"Antigravity","repair_for":["task-0001"]}]}'
Assert-Graph ($repairByCanonical.tasks[0].repair_for -eq 'task-0001') 'Canonical repair id must resolve directly'
$repairByTitle=ConvertFrom-AgexPlan -Existing @($existingTask) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair-title","title":"Repair title","objective":"Fix files","executor":"Antigravity","repair_for":["implement_utility_and_readme"]}]}'
Assert-Graph ($repairByTitle.tasks[0].repair_for -eq 'task-0001') 'Unique normalized title must resolve to canonical task'
$unknownRepair=$false;try{ConvertFrom-AgexPlan -Existing @($existingTask) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair-unknown","title":"Repair unknown","objective":"Fix","executor":"Antigravity","repair_for":["missing-target"]}]}'|Out-Null}catch{$unknownRepair=$_.Exception.Message -match "UNKNOWN TARGET 'missing-target'"}
...
$ambiguousRepair=$false;try{ConvertFrom-AgexPlan -Existing $ambiguousExisting -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair-ambiguous","title":"Repair ambiguous","objective":"Fix","executor":"Antigravity","repair_for":["shared_title"]}]}'|Out-Null}catch{$ambiguousRepair=$_.Exception.Message -match "AMBIGUOUS TARGET 'shared_title'"}
...
$recursiveRepair=ConvertFrom-AgexPlan -Existing @($existingTask,$repairExisting) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"repair-repair","title":"Repair repair","objective":"Fix again","executor":"Antigravity","repair_for":["repair-original"]}]}'
Assert-Graph ($recursiveRepair.tasks[0].repair_for -eq 'task-0002') 'Repair alias must resolve recursively'
$existingStatusBefore=$existingTask.Status;$invalidRepair=$false;try{ConvertFrom-AgexPlan -Existing @($existingTask) -Text '{"goal_status":"CONTINUE","tasks":[{"id":"bad-repair","title":"Bad repair","objective":"Fix","executor":"Antigravity","repair_for":["missing"]}]}'|Out-Null}catch{$invalidRepair=$true}
...
try {
    ...
    $proseClaim=Get-AgexTaskVerification ([pscustomobject]@{AffectedFiles=@('Convert-Names.ps1','README.md');Result='Created Convert-Names.ps1 and README.md. Convert-Names.ps1 was verified.'})
    ...
    $readmeEvidence=Get-AgexTaskVerification ([pscustomobject]@{AffectedFiles=@('README.md');Result='Updated README.md'})
    ...
    $badReadmeEvidence=Get-AgexTaskVerification ([pscustomobject]@{AffectedFiles=@('README.md');Result='Updated README.md'})
...
[void](Set-AgexTaskVerificationState -Entry $stateEntry -Verification $newPass -ExecutionSuccess $true -EvidenceAt (Get-Date))
[void](Set-AgexTaskVerificationState -Entry $stateEntry -Verification $oldFail -ExecutionSuccess $true -EvidenceAt (Get-Date).AddMinutes(-1))
...
$script:ui=New-AgexUiState -Project $Project -SessionId 'unblock-test';$prerequisite=[pscustomobject]@{Id='task-0001';Status='DONE';Dependencies=@()};$dependent=[pscustomobject]@{Id='task-0002';Status='BLOCKED';Dependencies=@('task-0001');Error='Unresolved dependency or cycle.';Reason=''};$downstream=[pscustomobject]@{Id='task-0003';Status='BLOCKED';Dependencies=@('task-0002');Error='Unresolved dependency or cycle.';Reason=''};Add-AgexUiTask -State $script:ui -Task $prerequisite;Add-AgexUiTask -State $script:ui -Task $dependent;Add-AgexUiTask -State $script:ui -Task $downstream
$unlocked=@(Unlock-AgexDependentTasks -TaskId 'task-0001')
Assert-Graph ($unlocked.Count -eq 1 -and $dependent.Status -eq 'QUEUED' -and (Test-AgexTaskDependenciesSatisfied -Entry $dependent)) 'Verified repair must unblock dependent task'
$dependent.Status='DONE';$unlocked=@(Unlock-AgexDependentTasks -TaskId 'task-0002')
Assert-Graph ($unlocked.Count -eq 1 -and $downstream.Status -eq 'QUEUED' -and (Test-AgexTaskDependenciesSatisfied -Entry $downstream)) 'Completed review must unblock downstream verification'
$script:ui=New-AgexUiState -Project $Project -SessionId 'mail-test'
...
function Invoke-AgexGraphExecutor { param($Agent,$Prompt,$WorkId); [void]$script:recipients.Add([pscustomobject]@{Agent=$Agent;Prompt=$Prompt;WorkId=$WorkId});$script:lastExecutorResult=$(if($script:testAnswer){$script:testAnswer}else{'Explicit answer'});$true }
Add-AgexOperationalMessages -Entry ([pscustomobject]@{Id='review';Agent='Codex';Result='{"messages":[{"to":"Antigravity","type":"QUESTION","content":"What changed?"}]}'})
Invoke-AgexMailbox -RootGoal 'Inspect' -WorkId 'mail-test'
...
Add-AgexOperationalMessages -Entry ([pscustomobject]@{Id='unicode-review';Agent='Codex';Result=$longMessageJson})
...
Invoke-AgexMailbox -RootGoal 'Inspect' -WorkId 'mail-unicode-test'
...
Add-AgexOperationalMessages -Entry ([pscustomobject]@{Id='review';Agent='Codex';Result=($duplicateReply+$duplicateReply)})
...
Add-AgexOperationalMessages -Entry ([pscustomobject]@{Id='review';Agent='Codex';Result='{"messages":[{"to":"Codex","type":"ANSWER","content":"self message"}]}'})
...
$script:ui.Tasks.Clear();Add-AgexUiTask -State $script:ui -Task $prior
Assert-Graph (Test-AgexTaskDependenciesSatisfied -Entry $repair) 'Repair task deadlocks behind the failed task it repairs'
$unrelated=[pscustomobject]@{Id='other';Status='REPAIR REQUIRED'}
$repair.RepairFor=@()
Assert-Graph (-not (Test-AgexTaskDependenciesSatisfied -Entry $repair)) 'Unrelated failed prerequisite incorrectly satisfied'
$script:ui=New-AgexUiState -Project $Project -SessionId 'final-reconcile-test';$script:cancellationSignal=@{Requested=$false};$script:ResolvedLeader='Codex'
...
Add-AgexUiTask -State $script:ui -Task $repaired
$reconciliationMessage=[pscustomobject]@{MessageId='unicode-evidence';From='Antigravity';To='Codex';TaskId='task-0005';Type='ANSWER';AuthoritativeBody=$script:testAnswer;PreviewBody=(Protect-AgexTelemetryText -Text $script:testAnswer);CreatedAt=(Get-Date).ToUniversalTime().ToString('o');DeliveredAt='';Status='ANSWERED';message_id='unicode-evidence';task_id='task-0005';content=(Protect-AgexTelemetryText -Text $script:testAnswer)};[void]$script:ui.Chat.Add($reconciliationMessage)
$reconciliationPayload=@(Get-AgexUiCollectionSnapshot -State $script:ui -Collection Chat)|ConvertTo-Json -Depth 5 -Compress
...
function Invoke-AgexGraphExecutor { param($Agent,$Prompt,$WorkId);$script:leaderPrompt=$Prompt;$script:lastExecutorResult='{"goal_status":"COMPLETE","reason":"All repaired evidence verified.","verification":"README bytes and utility verification passed.","tasks":[]}' -replace '\\','';$true }
Invoke-AgexGoalGraph -RootGoal 'Acceptance fixture' -WorkId 'final-reconcile-test'
