...
| Secrets | Stored with DPAPI / Keychain / Secret Service; never in settings, exports, logs or diagnostics; passed to MCP servers through environment variables only (tested). Without a Linux secret service AGEX refuses to store secrets. | OK |
